...
namespace cllio
{
	...
	struct _inline_serializer_utils
	{
		...
		inline static void _write_binary_uint16_t(byte_t*& out, const uint16_t value)
		{
			*out++ = static_cast<uint8_t>(value & 0xFF);
			*out++ = static_cast<uint8_t>((value >> 8) & 0xFF);
		}
		inline static void _write_binary_uint32_t(byte_t*& out, const uint32_t value)
		{
			*out++ = static_cast<uint8_t>(value & 0xFF);
			*out++ = static_cast<uint8_t>((value >> 8) & 0xFF);
		...
		inline static void _write_binary_uint64_t(byte_t*& out, const uint64_t value)
		{
			*out++ = static_cast<uint8_t>(value & 0xFF);
			*out++ = static_cast<uint8_t>((value >> 8) & 0xFF);
			*out++ = static_cast<uint8_t>((value >> 16) & 0xFF);
